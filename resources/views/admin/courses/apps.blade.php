        ...
        <div class="col-12">
          ...
          <div class="card">
            ...
            <button class="btn bg-gradient-primary text-white delete_all"> {{ __('site.Delete All Selected') }}</button>
                <h3 class="card-title">@lang('site.apps')</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped table-hover">
                ...
                <tbody>
                @foreach($course->apps as $i => $app)
                  <tr data-row-id='{{ $app->id }}'>
                      <td><input type="checkbox" name="apps[]" class="sub_chk" data-id="{{$app->id}}"></td>
                      <td>{{$i+1}}</td>
                      <td>{{$app->title}}</td>
                      ...
                      <td>
                          <div class="row">
                              <div class="col-md-4">
                                  <a class="edit btn bg-gradient-primary" data-toggle="modal" data-target="#update-app{{$i}}"><i class="fa fa-edit text-white"></i></a>
                              </div>
                              <div class="col-md-4">
                                  <a class="delete btn bg-gradient-danger " href="javascript:void(0)" data-delete-id="{{ $app->id }}"><i class="fa fa-trash text-white"></i></a>
                              </div>
                              <div class="col-md-4">
                                  <a class="btn bg-gradient-info text-white" href="{{url('/')}}/admin/apps/{{$app->id}}/student_apps" >أنشطه الطلاب</a>
                              </div>
                          </div>
                      </td>
                  </tr>
                  <div id="update-app{{$i}}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form method="post" action="{{url('/')}}/admin/apps/update/{{$app->id}}" enctype="multipart/form-data">
                                @csrf
...
<div id="add-app" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="post" action="{{url('/')}}/admin/apps/create" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="course_id" value="{{$course->id}}"/>
            <div class="modal-body">
                <div class="row">
