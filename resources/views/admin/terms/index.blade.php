...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>@lang('site.terms and condition')</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{url('/')}}/admin">@lang('site.admin_panel')</a></li>
              <li class="breadcrumb-item active">@lang('site.terms and condition')</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
      ...
      <div class="row">
        <div class="col-12">
          ...
          <div class="card">
            ...
            <a class="btn bg-gradient-primary text-white" href="{{url('/admin/terms/create')}}">@lang('site.add new term')</a>
            <button class="btn bg-gradient-primary text-white delete_all"> {{ __('site.Delete All Selected') }}</button>
                <h3 class="card-title">@lang('site.terms and condition')</h3>
            </div>
            <!-- /.card-header -->
              ...
              <table id="example1" class="table table-bordered table table-head-fixed table-striped table-hover">
                <thead>
                    <tr>
                        ...
                        <th>@lang('site.terms arabic title')</th>
                        {{--  <th>@lang('site.terms english title')</th>  --}}
                        <th>@lang('site.terms arabic description')</th>
                        {{--  <th>@lang('site.terms english description')</th>  --}}
                        <th> </th>
                    </tr>
                </thead>
                <tbody>
                @foreach($terms as $i => $term)
                  <tr data-row-id='{{ $term->id }}'>
                        <td><input type="checkbox" name="terms[]" class="sub_chk" data-id="{{$term->id}}"></td>
                        <td>{{$term->title_ar}}</td>
                        {{--  <td>{{$term->title_en}}</td>  --}}
                        <td>{!! $term->terms_ar !!}</td>
                        {{--  <td>{!! $term->terms_en !!}}}</td>  --}}
                        <td>
                            <div class="row">
                                <div class="col-md-6">
                                    <a class="edit btn bg-gradient-primary" href="{{ url('/admin/terms/'.$term->id.'/edit') }}"><i class="fa fa-edit text-white"></i></a>
                                </div>
                                <div class="col-md-6">
                                    <a class="delete btn bg-gradient-danger mr-1 ml-1" href="javascript:void(0)" data-delete-id="{{ $term->id }}"><i class="fa fa-trash text-white"></i></a>
                                </div>
                            </div>
                        </td>
                    </tr>
        ...
        Swal.fire({
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{!! url('admin/terms/delete_all' ) !!}",
                        type: 'POST',
