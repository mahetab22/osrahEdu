            ...
            <div class="sidebar-lesson">
                ...
                <div class="all-levels">
                    <div class="panel-group" id="accordion">
                        ...
                        <div class="panel-heading streaminglink">
                            <h4 class="panel-title">
                                <a href="{{$course->streamings_zoom[0]->presenter_url}} " class="zoom" target="_blank"><span>Zoom <i class="fa fa-tv"></i></span></a>
                            </h4>
                        </div>
@endif
                        <div class="all-buttons">
                            <a  href="https://zoom.us/oauth/authorize?response_type=code&client_id=CFcnxH_7SeiMz04zFK_g0w&redirect_uri=https://nhlzoom.nhledu.com/auth" id="meeting" class="add-btn addstr" >انشاء زوم بث مباشر</a>
                            @if(!empty($course->streamings_zoom[0]))
                            <a  value="{{$course->streamings_zoom[0]->class_id}}"   href="https://zoom.us/oauth/authorize?response_type=code&client_id=CFcnxH_7SeiMz04zFK_g0w&redirect_uri=https://nhlzoom.nhledu.com/auth" id="meeting_delete"  class="add-btn cancelstr" >الغاء زوم بث مباشر</a>
                                                ...
                                                <ul>
                                                    @foreach($levels->lessons as $lesson)
                                                        ...
                                                        @if(!empty($lesson->file))
                                                            <li>
                                                                <a href="{{ url('/') }}/public/storage/{{ $lesson->file }}" download><i class="far fa-file-pdf"></i> ملف مرفق مع &nbsp;{{ $lesson->title_ar }} </a>
                                                            </li>
                                                        @endif
                                                        @if($lesson->exam)
                                                            <li>
                                    ...
                                    <table class="table">
                                        ...
                                        @foreach($course->streamings_zoom as $streaming)
                                            ...
                                            <tr>
                                               ...
                                               {{-- <td class="actions">
                                                    <a href="{{$streaming->presenter_url}}" class="on-default btn btn-default drug" path="{{$streaming->class_id}}" type="submit"><i class="fa fa-times" aria-hidden="true"></i></a>
                                                </td>--}}
                                            </tr>
                    ...
                    <div class="add-qu form-contact">
                        ...
                        <!--<input name="presenter_id" value="{{ $course->supervisorcourse->supervisor->supervisorinfo->user_id }}" hidden="" />
                            ...
                            <div class="form-group">
                                ...
                                <select   name="attendee[]" id='pre-selected-optionss' multiple='multiple'>
                                    <label>@lang("site.Choose the received")</label>
                                    @foreach($course->subscriptioncourses as $subscriptioncourse)
                                    <option value='{{ $subscriptioncourse->user->id }}'>{{ $subscriptioncourse->user->name }}</option>
                                    @endforeach
    ...
    {{--
        ...
        <!-- Add streaming -->
        <div id="addstreaming-zoom" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <!-- Modal content-->
                <div class="modal-content">
                    <div class="modal-body">
                        ...
                        <div class="add-qu form-contact">
                            <h3>@lang("site.create live streaming")</h3>
                            <form method="get" name="form" action="{{route('auth-zoom')}}">
    ...
    {{--
                                ...
                                <input name="course_id" value="{{ $course->id }}" hidden="" >
                                <div class="form-group">
                                    <label>@lang("site.topic") *</label>
                                    <input type="text" placeholder="@lang('site.topic')" name="topic"   class="form-control" required=""/>
                                </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            $('.multiselect').each(function(index) {
                let multiselect_options = $(this).children('.multiselect_options');
                let multiselect_display = $(this).children('.multiselect_display');

                let total = multiselect_options.children('.multiselect_option').length - 1;
                var count = 0;
                multiselect_options.children('.multiselect_option').each(function(index){
                    let value = $(this).attr('value');
                    let this_checkbox = $(this).children('input[type="checkbox"]');
                    if (this_checkbox.prop('checked') && !$(this).hasClass('select_all')) {
                        count++;
                    }
                });

                if(count == 0) {
                    multiselect_display.children('.multiselect_display_text').text("لم يتم التحديد");
                    multiselect_options.children('.multiselect_option.select_all').children('input[type="checkbox"]').prop('checked', false);
                } else if (count >= total) {
                    multiselect_display.children('.multiselect_display_text').text("اختر الطالب");
                    multiselect_options.children('.multiselect_option.select_all').children('input[type="checkbox"]').prop('checked', true);
                } else {
                    multiselect_display.children('.multiselect_display_text').text("(" + count + " عدد الطالب المختارين)");
                    multiselect_options.children('.multiselect_option.select_all').children('input[type="checkbox"]').prop('checked', false);
                }
            });
        });

        $('.multiselect_option').on('click', function(){
        ...
        $('.multiselect_display').on('click', function() {
            var options = $(this).parent('.multiselect').children('.multiselect_options');
            let style = options.css("display") == "none" ? "block" : "none";
            options.css("display", style);
        });

        $(document).click(function(event) {
            $target = $(event.target);
            if((!$target.parent('.multiselect').children('.multiselect_options').length && !$target.parent('.multiselect_display').parent('.multiselect').children('.multiselect_options').length) && !$target.closest('.multiselect_options').length &&
                $('.multiselect_options').is(":visible")) {
                $('.multiselect_options').hide();
            }
        });

        $('.multiselect_option').on('click', function() {
            let select_all = $(this).hasClass('select_all');
            var multiselect_options = $(this).parent('.multiselect_options');
            if (select_all) {
                ...
                if (checked) {
                    multiselect_options.children('.multiselect_option').each(function(index){
                        ...
                        let this_checkbox = $(this).children('input[type="checkbox"]');
                        if (this_checkbox.prop('checked') && !$(this).hasClass('select_all')) {
                            $(this).attr('prev-checked', '1');
                        } else {
                            $(this).attr('prev-checked', '0');
                            this_checkbox.prop('checked', true);
                        }
                    });
                    multiselect_options.parent('.multiselect').children('.multiselect_display').children('.multiselect_display_text').text("اختر");
                } else {
                    let count = 0;
                    multiselect_options.children('.multiselect_option').each(function(index){
                        let previously_checked =  $(this).attr('prev-checked');
                        let this_checkbox = $(this).children('input[type="checkbox"]');
                        if (previously_checked != '1' || $(this).hasClass('select_all')) {
                            this_checkbox.prop('checked', false);
                        } else if (previously_checked == '1' && !$(this).hasClass('select_all')) {
                            count++;
                        }
                    });
                    if (count == 0) {
                        multiselect_options.parent('.multiselect').children('.multiselect_display').children('.multiselect_display_text').text("لم يتم التحديد");
                    } else {
                        multiselect_options.parent('.multiselect').children('.multiselect_display').children('.multiselect_display_text').text("(" + count + " عدد الطالب المختارين)");
                    }
                }
            } else {
                let total = multiselect_options.children('.multiselect_option').length - 1;
                ...
                if(count == 0) {
                    multiselect_options.parent('.multiselect').children('.multiselect_display').children('.multiselect_display_text').text("لم يتم اختيار اي طلاب");
                    multiselect_options.children('.multiselect_option.select_all').children('input[type="checkbox"]').prop('checked', false);
                } else if (count >= total) {
                    multiselect_options.parent('.multiselect').children('.multiselect_display').children('.multiselect_display_text').text("اختر الطلاب");
                    multiselect_options.children('.multiselect_option.select_all').children('input[type="checkbox"]').prop('checked', true);
                } else {
                    multiselect_options.parent('.multiselect').children('.multiselect_display').children('.multiselect_display_text').text("(" + count + " عدد الطالب المختارين)");
                    multiselect_options.children('.multiselect_option.select_all').children('input[type="checkbox"]').prop('checked', false);
