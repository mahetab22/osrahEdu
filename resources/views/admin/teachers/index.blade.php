      ...
      <div class="row">
        <div class="col-12">
          ...
          <div class="card">
            ...
            <a class="btn bg-gradient-primary text-white" href="{{url('/admin/teachers/create')}}">@lang('site.create new teacher')</a>
            <button class="btn bg-gradient-primary text-white delete_all"> {{ __('site.Delete All Selected') }}</button>
                <h3 class="card-title">@lang('site.teachers')</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped table-hover">
                <thead>
                    <tr>
                        ...
                        <th>@lang('site.teacher username')</th>
                        <th>@lang('site.teacher eductional')</th>
                        <th>@lang('site.teacher name')</th>
                        <th>@lang('site.teacher service name')</th>
                        <th> </th>
                    </tr>
                </thead>
                <tbody>
                @foreach($teachers as $i => $teacher)
                    <tr data-row-id='{{ $teacher->id }}'>
                        <td><input type="checkbox" name="teachers[]" class="sub_chk" data-id="{{$teacher->id}}"></td>
                        <td>{{ $teacher->user->name }}</td>
                        <td>{{ $teacher->Educational }}</td>
                        <td>{{ $teacher->name }}</td>
                        <td>{{ $teacher->service->title_ar}}</td>
                        ...
                        <td>
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <a class="delete btn bg-gradient-danger m-1" href="javascript:void(0)" data-delete-id="{{ $teacher->id }}"><i class="fa fa-trash text-white"></i></a>
                                </div>
                                <div class="col-md-3">
                                    <a class="edit btn bg-gradient-info text-white" href="{{ url('/admin/teacher/courses/'.$teacher->id) }}">{{ __('site.courses') }}</a>
                                </div>
                            </div>
                        </td>
                    </tr>
        ...
        Swal.fire({
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{!! url('admin/teacher/delete_all' ) !!}",
                        type: 'POST',
