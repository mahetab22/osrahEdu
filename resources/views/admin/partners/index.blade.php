        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">@lang('site.partners')</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
      ...
      <div class="row">
        <div class="col-12">
          <div class="card">
            ...
            <a class="btn bg-gradient-primary text-white" href="{{url('/admin/partners/create')}}">@lang('site.create new partner')</a>
            <button class="btn bg-gradient-primary text-white delete_all"> {{ __('site.Delete All Selected') }}</button>
                <h3 class="card-title">@lang('site.partners')</h3>
            </div>
            <!-- /.card-header -->
              ...
              <table id="example1" class="table table-bordered table table-head-fixed table-striped table-hover">
                <thead>
                    <tr>
                        ...
                        <th>@lang('site.partner title')</th>
                        <th>@lang('site.partner logo')</th>
                        <th> </th>
                    </tr>
                </thead>
                <tbody>
                @foreach($partners as $i => $partner)
                  <tr data-row-id='{{ $partner->id }}'>
                        <td><input type="checkbox" name="partners[]" class="sub_chk" data-id="{{$partner->id}}"></td>
                        <td>{{ $partner->title }}</td>
                        <td><img src="{{url('/')}}/{{$partner->logo}}" width="100px"/></td>
                        <td>
                            <div class="row">
                                <div class="col-md-6">
                                    <a class="edit btn bg-gradient-primary" href="{{ url('/admin/partners/'.$partner->id.'/edit') }}"><i class="fa fa-edit text-white"></i></a>
                                </div>
                                <div class="col-md-6">
                                    <a class="delete btn bg-gradient-danger mr-1 ml-1" href="javascript:void(0)" data-delete-id="{{ $partner->id }}"><i class="fa fa-trash text-white"></i></a>
                                </div>
                            </div>
                        </td>
                    </tr>
        ...
        Swal.fire({
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{!! url('admin/partners/delete_all' ) !!}",
                        type: 'POST',
