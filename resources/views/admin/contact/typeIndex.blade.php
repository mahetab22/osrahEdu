      ...
      <div class="row">
        <div class="col-12">
          ...
          <div class="card">
            ...
            <a class="btn bg-gradient-primary text-white"  data-toggle="modal" data-target="#add-type">@lang('site.create new')</a>
            <button class="btn bg-gradient-primary text-white delete_all"> {{ __('site.Delete All Selected') }}</button>
                <h3 class="card-title">@lang('site.contactType')</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped table-hover">
                ...
                @foreach($types as $i => $type)
                  <tr data-row-id='{{ $type->id }}'>
                      <td><input type="checkbox" name="types[]" class="sub_chk" data-id="{{$type->id}}"></td>
                      <td>{{$type->title_ar}}</td>
                      <td>{{$type->title_en}}</td>
                      <td>
                          <div class="row">
                          <div class="col-md-6">
                                  <a class=" btn bg-gradient-primary " data-toggle="modal" data-target="#edit-type{{$i}}"><i class="fa fa-edit text-white"></i></a>
                              </div>
                              <div class="col-md-6">
                                  <a class="delete btn bg-gradient-danger mr-1" href="javascript:void(0)" data-delete-id="{{ $type->id }}"><i class="fa fa-trash text-white"></i></a>
                              </div>
                          </div>
                      </td>
                  </tr>
        ...
        Swal.fire({
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{!! url('admin/contactType/delete_all' ) !!}",
                        type: 'POST',
